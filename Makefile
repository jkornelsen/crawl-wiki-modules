...
.PHONY: all test install clean clean-contrib clean-rltiles clean-android \
        ...
        monster json-data catch2-tests plug-and-play-tests \
...
JSON_DATA_OBJS=$(OBJECTS) util/json-data/json-data-main.o $(EXTRA_OBJECTS)
...
util/json-data/json-data-main.o: util/json-data/json-data-main.h

util/monster/monster: $(MONSTER_OBJS) $(CONTRIB_LIBS) dat/dlua/tags.lua
	+$(QUIET_LINK)$(CXX) $(LDFLAGS) $(MONSTER_OBJS) -o $@ $(LIBS)

util/json-data/json-data: $(JSON_DATA_OBJS) $(CONTRIB_LIBS) dat/dlua/tags.lua
	+$(QUIET_LINK)$(CXX) $(LDFLAGS) $(JSON_DATA_OBJS) -o $@ $(LIBS)

monster: util/monster/monster

json-data: util/json-data/json-data

test-monster: monster
	util/monster/monster quasit

test-json-data: json-data
	util/json-data/json-data

clean-monster:
	$(RM) util/monster/monster util/monster/vault_monster_data.h tile_info.txt

clean-json-data:
	$(RM) util/json-data/json-data util/json-data/json-data-main.o \
	util/json-data/json-data-main.d
